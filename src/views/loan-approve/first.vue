...
import {approveFirstList,loanQuery,approveFirstPass,approveEndReject, approveFirstReject} from "@/api/user"
...
const getList=()=>{
   approveFirstList(state.query).then((res)=>{
...
const handlePass=(row:any)=>{
        ...
        approveFirstPass({appId:row.id,loanIdL:row.id}).then((res)=>{
           if(res.data.code===20000){
           getList()
            ElMessage({
                type: 'success',
